...
class Analytics:
	...
	def get_sales_transactions_based_on_order_type(self):
		...
		q = frappe.qb.from_(doctype).select(
		...
		if self.filters.get("branch"):
			q = q.where(doctype.branch == self.filters.branch)

		self.entries = q.orderby(doctype.order_type).run(as_dict=True)
		self.get_teams()

	def get_sales_transactions_based_on_customers_or_suppliers(self):
		value_field = "base_net_total as value_field" if self.filters["value_quantity"] == "Value" else "total_qty as value_field"
		entity = "customer as entity" if self.filters.tree_type == "Customer" else "supplier as entity"
		entity_name = "customer_name as entity_name" if self.filters.tree_type == "Customer" else "supplier_name as entity_name"
		...
		if self.filters.get("branch"):
			filters["branch"] = self.filters.branch
	...
	def get_sales_transactions_based_on_items(self):
		...
		q = frappe.qb.from_(doctype_item).join(doctype).on(doctype.name == doctype_item.parent).select(
		...
		self.entries = q.run(as_dict=True)
		self.entity_names = {d.entity: d.entity_name for d in self.entries}
	...
	def get_sales_transactions_based_on_customer_or_territory_group(self):
		...
		self.entries = frappe.get_all(
			self.filters.doc_type, fields=[entity_field, value_field, self.date_field], filters=filters
		)
		self.get_groups()
	...
	def get_sales_transactions_based_on_project(self):
		...
		self.entries = frappe.get_all(
			self.filters.doc_type, fields=["project as entity", value_field, self.date_field], filters=filters
		)
	...
	def get_period(self, posting_date):
		if self.filters.range == "Weekly":
			return _("Week {0} {1}").format(posting_date.isocalendar()[1], posting_date.year)
		elif self.filters.range == "Monthly":
			return _("{0} {1}").format(self.months[posting_date.month - 1], posting_date.year)
		elif self.filters.range == "Quarterly":
			return _("Quarter {0} {1}").format(((posting_date.month - 1) // 3) + 1, posting_date.year)
		else:
			return str(get_fiscal_year(posting_date, company=self.filters.company[0])[0])
	...
	def get_groups(self):
		...
		parent = parent_map.get(self.filters.tree_type)
		self.depth_map = frappe._dict()
		self.group_entries = frappe.db.sql(
			f"""SELECT name, lft, rgt, {parent} as parent
			FROM `tab{self.filters.tree_type}` ORDER BY lft""", as_dict=1)
